...
export default function WarehouseSalesPage() {
  ...
  // ── Close (lock against further edits) ────────────────────────────────────

  const [closing, setClosing] = useState<string | null>(null);

  async function handleCloseInvoice(sale: WarehouseSale) {
    if (sale.balanceDue > 0.01) {
      toast.error(`Cannot close — balance of ${fmt(sale.balanceDue)} still owing. Record full payment first.`);
      return;
    }
    if (!window.confirm(`Close invoice #${sale.invoiceNumber}? Once closed it can no longer be edited (it can still be deleted).`)) return;
    setClosing(sale._id);
    try {
      const res = await fetch(`${BASE}/api/warehouse-sales/${sale._id}/close`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${getAuthToken()}` },
      });
      const json = await res.json();
      ...
      if (viewSale?._id === updated._id) setViewSale(updated);
      toast.success(`Invoice #${updated.invoiceNumber} closed`);
    } catch (err: any) {
      toast.error(err.message || 'Failed to close invoice');
    }
    setClosing(null);
  }
  ...
  function openEdit(sale: WarehouseSale) {
    if (sale.isClosed) { toast.error('This invoice is closed and can no longer be edited.'); return; }
    setEditSale(sale);
                ...
                <div key={s._id} className="bg-white rounded-xl p-4 shadow-sm border border-slate-100 flex items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {s.isClosed && (
                        <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium bg-slate-700 text-white">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <div className="mt-1 flex items-center gap-2 justify-end">
                      ...
                      {!s.isClosed && (
                        <button onClick={() => handleCloseInvoice(s)}
                          disabled={closing === s._id || s.balanceDue > 0.01}
                          title={s.balanceDue > 0.01 ? 'Full payment required to close' : 'Close invoice (no further edits)'}
                          className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-700 font-medium disabled:opacity-40 disabled:hover:text-slate-400">
                          {closing === s._id
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-100 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {viewSale.customerName} · {fmt(viewSale.totalAmount)}
                    {viewSale.isClosed && viewSale.closedAt && (
                      <> · closed {new Date(viewSale.closedAt).toLocaleDateString('en-NG')}{viewSale.closedByName ? ` by ${viewSale.closedByName}` : ''}</>
                    )}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {!viewSale.isClosed && (
                  <>
                    ...
                    <button onClick={() => handleCloseInvoice(viewSale)}
                      disabled={closing === viewSale._id || viewSale.balanceDue > 0.01}
                      title={viewSale.balanceDue > 0.01 ? 'Full payment required to close' : 'Close invoice (no further edits)'}
                      className="flex items-center gap-2 px-4 py-2 bg-slate-700 hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl text-sm font-bold transition-colors">
                      {closing === viewSale._id
                        ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />

-- ============================================================
-- ALLMAAJ COMPLETE DATABASE SCRIPT
-- Run this on a fresh database to set up everything from scratch.
-- Includes: schema, enums, tables, indexes, triggers, seed admin.
-- ============================================================


-- ============================================================
-- PART 1: WIPE EVERYTHING (fresh start)
-- ============================================================
...
CREATE TYPE product_unit         AS ENUM ('piece','kg','litre','box','carton','bag','roll','pair','set','dozen','yard','metre');
...
-- ── Products ──────────────────────────────────────────────────
-- is_cuttable: whether this product can be sold in partial lengths
-- unit_length_inches: full unit length in inches (e.g. 43 for a 43" stone slab)
CREATE TABLE products (
  ...
  unit_length_inches NUMERIC,
  created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at         TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- ── Branch Stock ──────────────────────────────────────────────
-- quantity is fractional for cuttable products:
--   e.g. 4.709 = 4 full pieces + 30.5" of a 43" unit
...
-- ── Daily Reports ─────────────────────────────────────────────
-- Must come BEFORE sales so the report_id FK on sales works
-- total_sales is auto-computed: cash + pos + unpaid + part
CREATE TABLE daily_reports (
  ...
  total_part_sales    NUMERIC(12,2) NOT NULL DEFAULT 0,
  total_sales         NUMERIC(12,2) GENERATED ALWAYS AS (
                        total_cash_sales + total_pos_sales + total_unpaid_sales + total_part_sales
                      ) STORED,
...
-- ── Sales ─────────────────────────────────────────────────────
-- items is a JSONB array. Each element:
-- {
--   product_id, product_name, item_type ('product'|'service'),
--   service_notes, quantity, unit_price, subtotal,
--   cut_length_inches, unit_length_inches, stock_deduct_qty
-- }
CREATE TABLE sales (
  id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  branch_id      UUID NOT NULL REFERENCES branches(id) ON DELETE RESTRICT,
...
CREATE INDEX idx_products_cuttable        ON products(is_cuttable) WHERE is_cuttable = true;
